...
Tomar defectos de un tamaño mínimo de 5 píxeles

ni tampoco conocer las condiciones experimentales utilizadas por el fabricante en el control de calidad de forma tal de realizar el análisis en las mismas condiciones y que en consecuencia dicho análisis pueda considerarse válido.
...
\singlespacing
\section{Asociación de incertezas y conteo de falsos negativos}
\spacing{1.5}

\hspace{0.5cm} El diámetro \underline{mínimo} de un defecto detectable debe ser mayor que el poder de resolución(R) del microscopio que viene dado por la fórmula teórica de Ernst Abbe y que es igual a:

\begin{equation}
R = \frac{\lambda}{2 . N.A.}
\end{equation},
donde $\lambda$ es la longitud de onda de la fuente de luz utilizada, que en este caso fue una fuente de luz blanca por lo que $\lambda = 0.55 \mu m$. y, N.A. es la apertura numérica del objetivo que fue de 0.25. En consecuencia, la resolución fue de 1.10$\mu m$. Además, considerando que el defecto mínimo detectable por el algoritmo debe ocupar por lo menos una longitud de 5 píxeles \cite{quentin} y que cada píxel de acuerdo a la calibración del microscopio es igual a 0.586 $\mu m$, se tiene que dicho diámetro es igual a \underline{3 $\mu m$}.
